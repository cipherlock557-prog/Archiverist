...
def load_data():
    ...
    try:
        ...
        print(f"⚠️ Could not load {DATA_FILE}: {error}")
...
async def custom_oc_render(owner, oc):
    try:
        ...
        print(f"⚠️ Custom OC layout failed: {e}")
        return None, None
...
class OCGiveView(discord.ui.View):
    ...
    @discord.ui.button(label="Give OC", emoji="🎁", style=discord.ButtonStyle.success)
...
class RoomSetupView(discord.ui.View):
    ...
    @discord.ui.button(label="Channel Counts", emoji="🔢", style=discord.ButtonStyle.primary)
    async def counts(self, interaction, button):
        await interaction.response.send_modal(RoomCountModal(self.member))

    @discord.ui.button(label="Name Text", emoji="📝", style=discord.ButtonStyle.secondary)
    async def text_names(self, interaction, button):
        ...
        if not state or state.text_count == 0:
            await interaction.response.send_message(
                "ℹ️ You selected 0 text channels.", ephemeral=True
            )
            return
        await interaction.response.send_modal(TextNamesModal(self.member))

    @discord.ui.button(label="Name Voice", emoji="🔊", style=discord.ButtonStyle.secondary)
    async def voice_names(self, interaction, button):
        state = pending_room_setups.get((self.member.guild.id, self.member.id))
        if not state or state.voice_count == 0:
            await interaction.response.send_message(
                "ℹ️ You selected 0 voice channels.", ephemeral=True
            )
            return
        await interaction.response.send_modal(VoiceNamesModal(self.member))

    @discord.ui.button(label="Create My Room", emoji="✅", style=discord.ButtonStyle.success, row=1)
...
async def create_room_setup_panel(member):
    ...
    if channel is None or not isinstance(channel, discord.TextChannel):
        print(
            f"⚠️ No room setup text channel is configured for {guild.name}. "
...
async def on_member_remove(member):
    ...
    if category is None:
        print(f"ℹ️ {member} left, but no personal room was found.")
        return
